...
describe Book do
  ...
  describe '#audiobook_download_slug' do
    ...
    before :each do
      @book = Fabricate(:book, :pretty_title => 'Les miserables')
    end
    ...
    describe 'when there is an audiobook correspondant to the book' do
      before :each do
        @audiobook = Fabricate(:audiobook, :pretty_title => 'Les miserables', :cached_slug => 'les-miserables-audiobook')
